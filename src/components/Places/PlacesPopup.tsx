...
function PlacesPopup({
  ...
  const [filter, setFilter] = useState<string | number>("Select");
  // const [rating, setRating] = useState<string | number>("");
  // const [user_rating_total, setUserRatingTotal] = useState<string | number>();
  ...
  useEffect(() => {
    if (open && selectedRow) {
      ...
      setInputs({
        ...
        g: selectedRow.g || "",
        youtube: selectedRow.youtube || "",
        video: selectedRow.video || "",
        // rating: selectedRow.rating || "",
        // user_rating_total: selectedRow.user_rating_total || "",
      ...
      setFilter(selectedRow.filter || "Select");
  ...
  const handleFilterChange = (ev: React.ChangeEvent<{ value: unknown }>) => {
    setFilter(ev.target.value as string);

    // const selCountry = commonList["filter"].filter(
    //   (ctry: any) => ctry.Ctry_Code === ev.target.value
    // );
  ...
  const handleSave = async (arrImgUrl?: string[]) => {
    ...
    const params = {
      ...
      geoLocation: FQGeoPoint(latitude, longitude),
      l: FQGeoPoint(latitude, longitude),
      g,
      youtube,
      video,
      // rating,
